...
function AdminEvents() {
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (selectedItemForEdit) {
                response = await axios.post("/api/portfolio/update-event", {
                    ...values,
                    _id: selectedItemForEdit._id,
                });
            } else {
                response = await axios.post("/api/portfolio/add-event", values);
            }
    ...
    const onDelete = async (item) => {
        try {
            dispatch(ShowLoading());
            const response = await axios.post("/api/portfolio/delete-event", {
                _id: item._id,
            });
            dispatch(HideLoading());
            if (response.data.success) {
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-4 gap-5 mt-5 sm:grid-cols-1">
                {events.map((event) => (
                    <div key={event._id} className="shadow border p-5 border-gray-400 flex flex-col">

                        <hr />
                        <h1>Event : {event.event}</h1>
                        <h1>Venue : {event.venue}</h1>
                        <h1>Date : {event.date}</h1>
                        <h1>Role : {event.role}</h1>
